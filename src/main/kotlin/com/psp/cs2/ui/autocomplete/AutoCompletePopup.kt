package com.psp.cs2.ui.autocomplete

import com.psp.cs2.ui.autocomplete.item.AutoCompleteClass
...
import com.runescape.cs2.ast.FunctionNode
...
class AutoCompletePopup(private val codeArea: CodeArea) : Popup() {
	...
	private fun focusedWord(highlight: Boolean = false): String? {
		...
		Cs2EditorController.VAR_LIST.clear()
		var match: String? = null
		if (matcher.find()) {
			match = matcher.group()
			if (highlight) {
				Cs2EditorController.VAR_LIST.add(match)
			}
		}
		codeArea.buildStyle()
		if (match != null) {
